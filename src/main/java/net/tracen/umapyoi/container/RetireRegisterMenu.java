...
public class RetireRegisterMenu extends AbstractContainerMenu {
    ...
    public List<UmaFactorStack> createResultFactors(ItemStack inputSoul, int ranking) {
        @NotNull
        Stream<UmaFactor> status = UmaFactorRegistry.REGISTRY.get().stream()
                .filter(fac -> fac.getFactorType() == FactorType.STATUS);
        long statusCount = UmaFactorRegistry.REGISTRY.get().stream()
        ...
        Stream<UmaFactor> extraStatus = UmaFactorRegistry.REGISTRY.get().stream()
                .filter(fac -> fac.getFactorType() == FactorType.EXTRASTATUS);
        long extraStatusCount = UmaFactorRegistry.REGISTRY.get().stream()
