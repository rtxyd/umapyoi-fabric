...
import net.tracen.umapyoi.client.model.pojo.FaceItem;
import net.tracen.umapyoi.client.model.pojo.FaceUVsItem;
import net.tracen.umapyoi.utils.MathUtils;
...
public class BedrockCube {
    ...
    public BedrockCube(float texOffX, float texOffY, float x, float y, float z, float width, float height, float depth,
        ...
        boolean xFlag = MathUtils.equalsFloat(width, 0.0f);
        boolean yFlag = MathUtils.equalsFloat(height, 0.0f);
        boolean zFlag = MathUtils.equalsFloat(depth, 0.0f);
    ...
    private BedrockPolygon getTexturedQuad(BedrockVertex[] positionsIn, float texWidth, float texHeight,
        ...
        if (MathUtils.equalsFloat(face.getUvSize()[0], 0.0F) && MathUtils.equalsFloat(face.getUvSize()[1], 0.0F))
            return null;
    ...
    public void compile(PoseStack.Pose pose, VertexConsumer consumer, int texU, int texV, float red, float green,
        ...
        for (BedrockPolygon polygon : this.polygons) {
            vector3f = new Vector3f(polygon.normal);
            ...
            for (BedrockVertex vertex : polygon.vertices) {
                vector4f = new Vector4f(this.vectors[vertex.posIndex], 1.f);
                matrix4f.transform(vector4f);
                consumer.vertex(vector4f.x(), vector4f.y(), vector4f.z(), red, green, blue, alpha, vertex.u, vertex.v,
                        texV, texU, nx, ny, nz);
            }
        }
    }
